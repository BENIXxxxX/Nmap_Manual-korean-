...
호스트 검색
    ...
    -PO 프로토콜 목록 (IP 프로토콜 핑)
        최신 호스트 검색 옵션 중 하나는 IP 헤더에 지정된 프로토콜 번호가 설정된 IP 패킷을 보내는 IP 프로토콜 핑입니다.
        프로토콜 목록은 앞에서 설명한 TCP, UDP 및 SCTP 호스트 검색 옵션의 포트 목록과 동일한 형식을 사용합니다.
        프로토콜이 지정되지 않은 경우 기본값은 여러 IP 패킷을 보내기위한 ICMP(프로토콜 1), IGMP(프로토콜 2) 및 IP-in-IP(프로토콜 4)입니다.
        기본 프로토콜은 컴파일시 nmap.h에서 DEFAULT_PROTO_PROBE_PORT_SPEC을 변경하여 구성 할 수 있습니다.
        ICMP,IGMP,TCP(프로토콜 6), UDP(프로토콜 17) 및 SCTP(프로토콜 132)의 경우 패킷은 올바른 프로토콜 헤더를 사용하여 전송되는 반면 다른 프로토콜은 IP 헤더 데이터 이외의 다른 프로토콜 없이 전송됩니다. (--data, --data-string 또는 --data-length 옵션이 지정됨)

        이 호스트 검색 방법은 프로브와 동일한 프로토콜을 사용하는 응답 또는 대상 호스트가 지정된 프로토콜을 지원하지 않음을 나타내는 ICMP 프로토콜에 연결 할 수 없음 메시지를 찾습니다.
        두 유형의 응답은 대상 호스트가 활성 상태임을 나타냅니다.

    --disable-arp-ping (No ARP or ND Ping)
        nmap은 일반적으로 -Pn 또는 -PE와 같은 다른 호스트 검색 옵션이 사용되는 경우에도 로컬로 연결된 이더넷 호스트에서 ARP 또는 IPv6 인접 검색(ND) 검색을 수행합니다.
        이 암시적 동작을 비활성화하려면 --disable-arp-ping 옵션을 사용합니다.

        기본 동작은 일반적으로 덛 빠르지만, 이 옵션은 모든 대상이 실행중인 것처럼 보이도록 ARP 스캔을 기반으로 라우터가 모든 ARP 요청에 추측적으로 응답하는 프록시 ARP를 사용하는 네트워크에서 유용합니다.

    --discovery-ignore-rst
        경우에 따라 방화벽은 사용되지 않거나 허용되지 않는 주소 감지에 대한 응답으로 TCP 재설정 (RST) 응답을 스푸핑 할 수 있습니다.
        nmap은 일반적으로 RST 응답을 대상이 시작되었다는 증거로 취급하므로 존재하지 않는 대상을 검색하는 데 시간이 낭비 될 수 있습니다.
        --discovery-ignore-rst 를 사용하면 nmap이 호스트 검색 중에 이러한 응답을 고려하지 않습니다.
        이 경우 대상을 놓치지 않도록 추가 호스트 검색 옵션을 선택해야 할 수 있습니다.
        
    --traceroute (호스트 경로 추적)
        스캔 결과의 정보를 사용하여 스캔 후 추적 라우팅을 수행하여 대상에 도달할 가능성이 가장 높은 포트 및 프로토콜을 판별하십시오.
        연결 검색(-sT) 및 유휴 검색 (-sI)을 제외한 모든 검색 유형에 적용됩니다. 모든 추적은 nmap의 동적 타이밍 모델을 사용하며 병렬로 실행됩니다.

        traceroute의 작동 원리는 스캐너와 대상 호스트 사이의 중간 홉에서 ICMP 시간 초과 메시지를 유도하기 위해 TTL(Time to Live)이 낮은 패킷을 보내는 것입니다.
        표준 추적 경로 구현은 TTL 1로 시작하여 대상 호스트에 도달 할 때까지 TTL을 증가시킵니다.
        nmap의 traceroute는 높은 TTL로 시작한 다음 0에 도달 할 때까지 TTL을 감소시킵니다.
        거꾸로하면 nmap이 영리한 캐싱 알고리즘을 사용하여 여러 호스트의 추적 속도를 높일 수 있습니다.
        네트워크 상태에 따라 nmap은 호스트 당 평균적으로 5-10개의 패킷을 덜 보냅니다.
        단일 서브넷이 스캔되는 경우(예: 192.168.0.0/24) nmap은 대부분의 호스트에 두 개의 패킷만 보내면됩니다.
